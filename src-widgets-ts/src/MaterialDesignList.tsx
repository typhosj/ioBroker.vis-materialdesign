...
import { squarePreview, RenderProps, VisWidget, createInfo, iconField, parseActionValue, setStateValue, sizeCss, stateValue, sanitizeHtml } from './widgetUtils';
...
const attrs: RxWidgetInfo['visAttrs'] = [
    ...
    { name: 'listItemData', label: 'group_listItemData', fields: [{ name: 'listItemDataMethod', label: 'listItemDataMethod', type: 'select', options: ['inputPerEditor', 'jsonStringObject'], default: 'inputPerEditor' }, { name: 'countListItems', label: 'countListItems', type: 'number', default: 1, onChange: (_f, data, change) => { change({ ...data, lastListItemIndex: Math.max(0, Math.floor(n(data.countListItems, 1)) - 1) }); return Promise.resolve(); } }, { name: 'lastListItemIndex', type: 'number', default: 0, hidden: () => true }, { name: 'json_string_oid', label: 'json_string_oid', type: 'id' }] },
