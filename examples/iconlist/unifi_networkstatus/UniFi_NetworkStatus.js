...
Dieses Skript erzeugt einen json string um den Netzwerk Status aller Geräte des Unifi Adapters mithilfe des Material Design IconList Widget anzuzeigen.
=========================================================================================================================================================================
...
* 1.0.1:            Datenpunkte für Anzahl LAN, WLAN, Gäste hinzugefügt
                    Unterscheidung zwischen On und Off Time hinzugefügt
...
let dpLanDevices = '0_userdata.0.vis.NetzwerkDevicesStatus.lanDevices'          // Datenpunkt Anzahl LAN Geräte verbunden / nicht verbunden (Typ: Zeichenkette (String))
let dpWlanDevices = '0_userdata.0.vis.NetzwerkDevicesStatus.wlanDevices'        // Datenpunkt Anzahl WLan Geräte verbunden / nicht verbunden (Typ: Zeichenkette (String))
let dpGuestDevices = '0_userdata.0.vis.NetzwerkDevicesStatus.guestDevices'      // Datenpunkt Anzahl Gäste Geräte verbunden / nicht verbunden (Typ: Zeichenkette (String))
...
let imagePath = '/vis.0/myImages/devices/';                                     // Pfad zu den verwendeten Bildern
...
function createList() {
    try {
        ...
        let countLanDevicesOn = 0;
        let countLanDevicesOff = 0;
        let countWlanDevicesOn = 0;
        let countWlanDevicesOff = 0;
        let countGuestOn = 0;
        let countGuestOff = 0;
        ...
        for (var i = 0; i <= devices.length - 1; i++) {
            ...
            if (isInRange(lastSeen) === true) {
                ...
                let isGuest = existsState(idDevice + ".is_guest") ? getState(idDevice + ".is_guest").val : false;
                ...
                if (isWired) {
                    ...
                    if (swPort < 25 && isWired === true) {
                        ...
                        image = (note && note !== null && note.image) ? `${imagePath}${note.image}.png` : `${imagePath}lan_noImage.png`
                    ...
                    image = (note && note !== null && note.image) ? `${imagePath}${note.image}.png` : `${imagePath}wlan_noImage.png`
                ...
                function addToList() {
                    ...
                    let subText = `
                                ...
                                <div style="display: flex; flex-direction: row; padding-left: 8px; padding-right: 8px; margin-top: 5px; align-items: center;">
                    ...
                    if (isConn === false) {
                        ...
                        offTime = now.diff(start);
                        betriebszeit = offTime * -1;
                    } else {
                        offTime = betriebszeit * -1;
                    }
        ...
        } else if (sortMode === 'connected' || sortMode === 'empfangen' || sortMode === 'gesendet' || sortMode === 'erlebnis' || sortMode === 'betriebszeit' || sortMode === 'offTime') {
            deviceList.sort(function (a, b) {
        ...
        if (existsState(dpLanDevices)) {
            setState(dpLanDevices, `${countLanDevicesOn} / ${countLanDevicesOff}`, true)
        }

        if (existsState(dpWlanDevices)) {
            setState(dpWlanDevices, `${countWlanDevicesOn} / ${countWlanDevicesOff}`, true)
        }

        if (existsState(dpGuestDevices)) {
            setState(dpGuestDevices, `${countGuestOn} / ${countGuestOff}`, true)
        }
    ...
    function getOnOffTime(isConnected, betriebszeit, lastSeen) {
        if (isConnected) {
            return `<span style="color: gray; font-size: ${offlineTextSize}px; line-height: 1.5; font-family: RobotoCondensed-LightItalic;">online seit ${moment.duration(betriebszeit, 'seconds').format(durationFormat, 0)}</span>`
        } else {
            let now = moment(new Date());
            let start = moment(lastSeen * 1000);
            return `<span style="color: gray; font-size: ${offlineTextSize}px; line-height: 1.5; font-family: RobotoCondensed-LightItalic;">offline seit ${moment.duration(now.diff(start)).format(durationFormat, 0)}</span>`
        }
    }
}
